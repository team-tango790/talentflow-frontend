...
/* ── API Functions — all use Bearer token via getHeaders() ── */
...
function mapAssignment(raw: any): ApiAssignment {
  ...
  const mentorName: string = raw.mentor?.name ?? raw.mentorName ?? "Mentor";
  ...
  return (
    <div style={{ position: "relative", width: 100, height: 100 }}>
      <svg width="100" height="100" viewBox="0 0 100 100" style={{ transform: "rotate(-90deg)" }}>
        ...
        <circle cx="50" cy="50" r={r} fill="none" stroke="#d4a93d" strokeWidth="7" strokeDasharray={`${p} ${c - p}`} strokeLinecap="round" />
      </svg>
...
function SuccessModal({ onDashboard, onViewSubmission }: { onDashboard: () => void; onViewSubmission: () => void }) {
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(17,41,32,0.75)", backdropFilter: "blur(2px)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ background: "white", borderRadius: 24, padding: "44px 40px", width: "100%", maxWidth: 420, textAlign: "center", boxShadow: "0 24px 80px rgba(0,0,0,0.4)", animation: "modalIn 0.45s cubic-bezier(0.34,1.56,0.64,1) forwards" }}>
        <div style={{ width: 90, height: 90, borderRadius: "50%", background: "#fef9ec", border: "2px solid #f0d070", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 24px" }}>
          <svg width="42" height="42" viewBox="0 0 42 42" fill="none"><circle cx="21" cy="21" r="19" stroke="#E9BD55" strokeWidth="2" /><path d="M12 21l7 7 11-13" stroke="#E9BD55" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
        </div>
...
/* ── Submission Form ── */
function TaskSubmissionPage({ assignment, isResubmit, onBack, onSubmit }: { assignment: ApiAssignment; isResubmit: boolean; onBack: () => void; onSubmit: () => void }) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const payload: SubmissionPayload = { submissionType: "FILE", fileUrl: fileUrl || undefined, figmaUrl: figmaUrl || undefined, portfolioUrl: portfolioUrl || undefined, noteToMentor: noteToMentor || undefined };
  ...
  const inputStyle = { width: "100%", height: 50, border: "1.5px solid #d8e4e0", borderRadius: 10, padding: "0 16px", fontSize: 14, color: "#112920", background: "white", outline: "none", fontFamily: "'DM Sans', sans-serif" };
    ...
    <div style={{ display: "flex", minHeight: "100vh", background: "#f0ede4", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ flex: 1, overflow: "hidden" }}>
        <main style={{ flex: 1, overflowY: "auto", padding: "40px 56px", maxWidth: 860 }}>
          ...
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 26, fontWeight: 700, color: "#112920", marginBottom: 6 }}>{isResubmit ? "Edit Submission" : "Task Submission"}</h1>
          <p style={{ color: "#b8cec8", fontSize: 12, marginBottom: 36 }}>For: <strong style={{ color: "#112920" }}>{assignment.title}</strong></p>
          {error && <div style={{ background: "#fdf2f1", border: "1px solid #f5a9a3", borderRadius: 8, padding: "12px 16px", marginBottom: 24, fontSize: 13, color: "#c0392b" }}>✕ {error}</div>}
          <div style={{ display: "flex", flexDirection: "column", gap: 20, marginBottom: 36 }}>
            {[
              ...
              <div key={label}>
                ...
                <input type="url" placeholder={placeholder} value={value} onChange={(e) => onChange(e.target.value)} style={inputStyle} />
              </div>
            ))}
            <div>
              <label style={{ display: "block", fontSize: 13, fontWeight: 600, color: "#112920", marginBottom: 8 }}>Note to Mentor <span style={{ fontWeight: 400, color: "#9ab0aa" }}>(optional)</span></label>
              <textarea placeholder="Any context for your mentor..." value={noteToMentor} onChange={(e) => setNoteToMentor(e.target.value)} rows={3} style={{ ...inputStyle, height: "auto", padding: "12px 16px", resize: "vertical", lineHeight: 1.6 }} />
            </div>
          </div>
          <button onClick={handleSubmit} disabled={submitting} style={{ width: "100%", height: 54, borderRadius: 12, background: submitting ? "#3d6b58" : "#112920", color: "white", border: "none", cursor: submitting ? "not-allowed" : "pointer", fontSize: 15, fontWeight: 600, fontFamily: "'DM Sans', sans-serif", display: "flex", alignItems: "center", justifyContent: "center", gap: 10 }}>
            {submitting ? (<><span className="spinner" />{isResubmit ? "Updating…" : "Submitting…"}</>) : (isResubmit ? "Update Submission" : "Submit Assignment")}
          </button>
        </main>
      </div>
    </div>
...
/* ── Submission Detail ── */
function SubmissionDetailPage({ assignment, onBack, hasFeedback, onResubmit }: { assignment: ApiAssignment; onBack: () => void; hasFeedback: boolean; onResubmit: () => void }) {
      ...
      <div style={{ flex: 1, overflow: "hidden" }}>
        <main style={{ flex: 1, overflowY: "auto", padding: "40px 48px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 28 }}>
            <button onClick={onBack} style={{ display: "flex", alignItems: "center", gap: 8, background: "none", border: "none", cursor: "pointer", color: "rgba(255,255,255,0.55)", fontSize: 13 }}>
            ...
            {canResubmit && (
              <button onClick={onResubmit} style={{ display: "flex", alignItems: "center", gap: 6, background: "rgba(255,255,255,0.08)", border: "1px solid rgba(255,255,255,0.15)", borderRadius: 8, cursor: "pointer", color: "rgba(255,255,255,0.75)", fontSize: 13, fontWeight: 500, padding: "8px 14px" }}>
          ...
          <div style={{ background: "#f7f5ef", borderRadius: 16, overflow: "hidden", marginBottom: 16, borderLeft: "4px solid #c9a227" }}>
            <div style={{ padding: "28px 28px 24px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                <h2 style={{ fontFamily: "'Playfair Display', serif", fontSize: 22, fontWeight: 700, color: "#112920" }}>{assignment.title}</h2>
            ...
            <div style={{ borderTop: "1px solid #e4e0d6", display: "flex" }}>
...
export default function AssignmentsPage() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // Fetch assignments from enrolled courses + submissions (graded/submitted ones)
      ...
      // Merge: prefer submission version for assignments that have been submitted
      const submittedIds = new Set(subList.map(s => s.id));
      const unsubmittedAssignments = assignList.filter(a => !submittedIds.has(a.id));
      const merged = [...subList, ...unsubmittedAssignments];

      if (merged.length > 0) {
        setAssignments(merged);
      } else {
        setAssignments(FALLBACK);
        setError("No assignments found — showing demo data.");
      }
    } catch {
      setAssignments(FALLBACK);
  ...
  if (detailLoading) return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f5f0", fontFamily: "'DM Sans', sans-serif" }}>
      <style>{globalStyles}</style><Sidebar />
  ...
  return (
    <div style={{ display: "flex", minHeight: "100vh", background: "#f5f5f0", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "hidden" }}>
        <header style={{ height: 60, background: "white", borderBottom: "1px solid #e8eeec", display: "flex", alignItems: "center", justifyContent: "space-between", padding: "0 32px", flexShrink: 0 }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, background: "#f5f5f0", borderRadius: 8, padding: "8px 14px", border: "1px solid #e0e8e4", width: 320 }}>
        ...
        <main style={{ flex: 1, overflowY: "auto", padding: "36px 40px" }}>
          <h1 style={{ fontFamily: "'Playfair Display', serif", fontSize: 28, fontWeight: 700, color: "#112920", marginBottom: 6 }}>My Assignments</h1>
          <p style={{ color: "#7a9a91", fontSize: 14, marginBottom: 28 }}>Track, manage, and submit all your course assignments and evaluations</p>

          {error && (
            <div style={{ background: "#fffbeb", border: "1px solid #fcd34d", borderRadius: 8, padding: "10px 16px", marginBottom: 20, fontSize: 13, color: "#92400e", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
              <span>⚠ {error}</span>
              <button onClick={load} style={{ background: "none", border: "none", cursor: "pointer", color: "#d97706", fontWeight: 600, fontSize: 13 }}>Retry</button>
            </div>
          )}

          <div style={{ display: "flex", gap: 6, marginBottom: 28 }}>
            {(["All", "Due soon", "Submitted", "Graded"] as FilterTab[]).map(tab => (
              <button key={tab} onClick={() => setActiveTab(tab)} style={{ padding: "8px 20px", borderRadius: 24, border: "none", fontSize: 13, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", background: activeTab === tab ? "#112920" : "transparent", color: activeTab === tab ? "white" : "#7a9a91", transition: "all 0.2s" }}>
                {tab}
              </button>
            ))}
          </div>
          ...
          {!loading && (
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              {filtered.length === 0 && <div style={{ textAlign: "center", padding: "60px 0", color: "#7a9a91", fontSize: 14 }}>No assignments in this category yet.</div>}
              {filtered.map(a => {
                const s = STATUS_STYLES[a.status];
                return (
                  <div key={a.id} className="assignment-card" style={{ background: "white", borderRadius: 12, border: "1px solid #e8eeec", borderLeft: `4px solid ${LEFT_BORDER[a.status]}`, display: "flex", alignItems: "stretch", overflow: "hidden" }}>
                    <div style={{ flex: 1, padding: "22px 24px" }}>
                      ...
                      <h3 style={{ fontFamily: "'Playfair Display', serif", fontSize: 18, fontWeight: 700, color: "#112920", marginBottom: 8 }}>{a.title}</h3>
                      <p style={{ fontSize: 13, color: "#7a9a91", lineHeight: 1.6, marginBottom: 14, maxWidth: 620 }}>{a.description}</p>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", gap: 10, padding: "22px 24px", borderLeft: "1px solid #f0f4f2" }}>
                      {(a.action === "submit" || a.action === "start") && (
                        <button className="btn-dark" onClick={() => handleAction(a)} style={{ background: "#112920", color: "white", border: "none", borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap" }}>
                          {a.action === "submit" ? "Submit now" : "Start submission"}
                        </button>
                      )}
                      {a.action === "feedback" && (
                        <button className="btn-outline" onClick={() => handleAction(a)} style={{ background: "white", color: "#112920", border: "1.5px solid #dce6e2", borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap" }}>View feedback</button>
                      )}
                      {a.action === "view" && (
                        <button className="btn-outline" onClick={() => handleAction(a)} style={{ background: "white", color: "#112920", border: "1.5px solid #dce6e2", borderRadius: 8, padding: "10px 20px", fontSize: 13, fontWeight: 500, cursor: "pointer", whiteSpace: "nowrap" }}>View submission</button>
                      )}
                    </div>
                  </div>
                );
