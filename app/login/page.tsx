...
function saveAuthData(token: string, user: AuthSuccessResponse["user"]) {
...
export default function Login() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (response.ok) {
        const data = await safeJson<AuthSuccessResponse>(response);
        if (data?.token && data?.user) saveAuthData(data.token, data.user);
      ...
      if (response.status === 401)
        ...
        setGlobalError("No account found with this email. Please sign up.");
  ...
  //
  // Flow:
  //   1. Google Identity Services SDK opens the account picker popup.
  //   2. On success, Google returns a credential (ID token).
  //   3. We POST that token to /api/auth/google-auth.
  //   4. Backend verifies the token, finds/creates the user, returns { token, user }.
  //
  // Setup (one-time):
  //   a) Add to .env.local:
  //        NEXT_PUBLIC_GOOGLE_CLIENT_ID=your-client-id.apps.googleusercontent.com
  //   b) Add to your root layout <head>:
  ...
  const handleGoogleLogin = () => {
    ...
    // `google` is injected globally by the GSI script
      ...
      callback: async (googleResponse: { credential: string }) => {
        try {
          const res = await fetch(`${BASE_URL}/api/auth/google-auth`, {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            // Send the raw Google ID token — backend verifies it server-side
          ...
          if (res.status === 403)
            setGlobalError("This Google account is not authorised. Please contact support.");
          else if (res.status >= 500)
            setGlobalError("Server error. Please try again in a moment.");
          else
            setGlobalError(serverMsg ?? "Google sign-in failed. Please try again.");
    ...
    google.accounts.id.prompt((notification: any) => {
      // User dismissed the popup without selecting an account
      ...
      <style>{`
        * { box-sizing: border-box; }
        ...
        .tf-label {
          ...
          color: #2d4a42; margin-bottom: 6px; letter-spacing: 0.01em;
        }
        ...
        .spinner {
          width: 16px; height: 16px; border: 2px solid #dce6e2;
          border-top-color: #112920; border-radius: 50%;
        ...
        .forgot-link {
          ...
          color: #E9BD55; text-decoration: none; font-weight: 500; transition: color 0.2s;
        }
      ...
      <main style={{ background: "#fafcfb" }} className="flex justify-center items-center flex-1 min-h-screen px-6 py-12">
        <div style={{ width: "100%", maxWidth: 420 }}>
          ...
          <div className="animate-in" style={{ marginBottom: 32 }}>
            ...
            <p style={{ color: "#7a9a91", fontSize: 14, lineHeight: 1.6 }}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column" }}>
              ...
              <div className="tf-input-wrap">
                <input
                  id="password"
                  className={`tf-input tf-input-password${fieldErrors.password ? " error" : ""}`}
            ...
            <div className="animate-in animate-in-delay-2" style={{ marginTop: 10 }}>
              ...
              <p style={{ textAlign: "center", fontSize: 13, color: "#7a9a91", margin: "14px 0" }}>
                ...
                <Link href="/register" style={{ color: "#E9BD55", fontWeight: 500, textDecoration: "none" }}>
                  Sign up
                </Link>
              </p>
